...
def fetch_pending_videos():
    """Busca até 10 vídeos com status 'pending' equilibrando 5 de futebol e 5 de política."""
    query = """
    (
...
def process_single_video(video: dict):
    ...
    # 6. Notifica o servidor para rodar o pipeline de IA imediatamente
    _log(f'Acionando IA (Whisper + LLaMA + FFmpeg) no servidor...')
    run_remote_cmd("docker exec clip-processor python -c 'from src.rss_poller import poll_all_channels; poll_all_channels()'")
