...
## Corpus Check
- 445 files · ~790,098 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 3240 nodes · 5740 edges · 244 communities (216 shown, 28 thin omitted)
- Extraction: 98% EXTRACTED · 2% INFERRED · 0% AMBIGUOUS · INFERRED: 103 edges (avg confidence: 0.75)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `dee8ff09`
...
- components.json
- 01-03-PLAN: Gerar .env, subir Docker, aplicar schema
- download_video
...
- pipeline_runner.py
- Quota, window and round-robin logic
...
Cohesion: 0.10
Nodes (28): publish_pending_clips(), datetime, Publica clips prontos e retorna quantidade publicada. Fluxo multi-canal (Phase…, dt_sp(), make_conn_with_clips(), make_mock_uploader(), Upload bem-sucedido: pending → publishing → published., Upload com erro: status vai para 'failed', quota não é incrementada. (+20 more)
...
Cohesion: 0.21
Nodes (10): check_disk_space(), check_youtube_tokens(), _log(), Verifica se todos os canais destino ativos possuem arquivo de credencial OAuth…, Executa o ciclo completo de monitoramento do Watchdog., Verifica espaço livre em disco no volume de vídeos., run_watchdog_cycle(), test_watchdog.py — Testes unitários para o módulo watchdog.py (monitoramento e… (+2 more)

### Community 8 - "SourceChannel"
Cohesion: 0.18
Nodes (7): Illuminate\Database\Console\Seeds\WithoutModelEvents, Illuminate\Database\Seeder, SourceChannelController, SourceChannel, BaselineSeeder, DatabaseSeeder, SourceChannelsSeeder
...
Cohesion: 0.16
Nodes (24): internal_api.resolve_channel(url) — yt-dlp channel resolution, _check_auth(), purge_old_videos(), internal_api.py — Sidecar HTTP interno consumido pelo painel Laravel.…, Limpa vídeos fonte com published_at anterior a `before_date` (formato 'YYYY-MM-…, Roda yt-dlp em modo metadata-only e extrai id/name/handle. Padrão yt-dlp:…, Dispara um ciclo imediato de publicação de clipes aprovados., Chama src.rejeitar.rejeitar(clip_id) diretamente. Preserva exit codes 0/1/2. (+16 more)

### Community 11 - "selector.py"
Cohesion: 0.17
Nodes (14): _enforce_longform_duration(), _filter_shortform_duration(), _log(), _normalize_scores(), selector.py — Seleção de momentos via IA com fallback automático. Prioridade em…, Converte scores 0–1 (comum no Groq) para escala 0–10 do pipeline., Seleciona momentos via Groq (fallback sempre disponível)., Garante duração mínima de MIN_LONGFORM_SECONDS e máxima de MAX_LONGFORM_SECONDS… (+6 more)

### Community 12 - "DestinationChannels.tsx"
Cohesion: 0.08
Nodes (37): ChannelTemplateModal(), COLOR_PRESETS, Props, TemplateConfig, Niche, NicheCombobox(), slugify(), Command() (+29 more)
...
### Community 15 - "cn"
Cohesion: 0.05
Nodes (58): Avatar(), AvatarBadge(), AvatarGroup(), AvatarGroupCount(), AvatarImage(), Breadcrumb(), BreadcrumbEllipsis(), BreadcrumbItem() (+50 more)
...
### Community 18 - "GeneratedClip"
Cohesion: 0.10
Nodes (16): POST /internal/resolve-channel (sidecar endpoint), Filament Resource GET/HEAD-only routing pattern, App\Filament\Resources\SourceChannelResource, CreateSourceChannel Page, GeneratedClip, AjudaCommand, AprovarCommand, ClipesCommand (+8 more)
...
Cohesion: 0.11
Nodes (15): Credentials, HttpError, MediaFileUpload, Exception, uploader.py — Upload de clips para YouTube Data API v3. Exporta: -…, RefreshError, Default privacyStatus=private, configuravel por YOUTUBE_PRIVACY_STATUS, destination_channels.oauth_expired_flag column (+7 more)

### Community 21 - "generate_metadata"
Cohesion: 0.14
Nodes (19): Anthropic structured outputs via output_config json_schema, _build_prompt(), generate_metadata(), _generate_via_anthropic(), _generate_via_groq(), _log(), _normalize_metadata(), metadata_generator.py — Geração de título, descrição e tags para YouTube.… (+11 more)
...
Cohesion: 0.04
Nodes (34): Phase 8 Deferred Items, BOT-01 requirement (webhook + allowlist + dedup), BOT-02 requirement (6 Telegram commands), BOT-03 requirement (pipeline-event notifications), client(), fixture, RED tests for internal_api sidecar (implementação GREEN no Plan 08-07)., POST /internal/process-url com format='longo' repassa fmt='longo' pro… (+26 more)
...
### Community 28 - "01-03-PLAN: Gerar .env, subir Docker, aplicar schema"
Cohesion: 0.40
Nodes (5): ANTHROPIC_API_KEY intencionalmente vazia até Phase 3, .env (secrets reais), 01-03-PLAN: Gerar .env, subir Docker, aplicar schema, 01-03-SUMMARY: infra Docker operacional, Whisper local removido do docker-compose
...
Cohesion: 0.08
Nodes (6): Illuminate\Http\RedirectResponse, DashboardController, ProcessVideoController, SourceVideoController, SourceVideo, ClipProcessorClient

### Community 31 - "Offers.tsx"
Cohesion: 0.06
Nodes (35): CtaHint(), GenerateCopyButton(), OfferCopy, Source, Popover(), PopoverContent(), PopoverDescription(), PopoverHeader() (+27 more)

### Community 32 - "Inertia\Response"
Cohesion: 0.11
Nodes (10): Illuminate\Support\Facades\Storage, Inertia\Response, DocumentationController, NicheController, OfferPerformanceController, TranscriptionController, UsefulLinksController, Niche (+2 more)
...
Cohesion: 0.13
Nodes (18): _build_clip_context(), _fetch_clip(), process_clip(), video_processor.py — Corte, legendas, thumbnail e processamento de clips.…, Busca a imagem de background 1920x1080 do canal ou nicho., Processa um registro de generated_clips com status pending_cut. Pipeline: cut →…, Gera configuração padrão inteligente de template 9:16 baseada no nicho do canal., resolve_background_path() (+10 more)
...
Cohesion: 0.07
Nodes (37): Accordion(), AccordionContent(), AccordionItem(), AccordionTrigger(), Badge(), badgeVariants, Card(), CardContent() (+29 more)

### Community 45 - "ARCHITECTURE.md (as-built, commit dca6e44)"
Cohesion: 0.12
Nodes (18): ARCHITECTURE.md (as-built, commit dca6e44), AI fallback chain (Claude → Groq → deterministic), APScheduler jobs (ingest_cycle, publish_cycle, clip_pending_ttl), clip-processor service (as-built), Dead code: painel/app/Filament/Pages/Dashboard.php orphan, env() outside config() pitfall in DashboardController, generated_clips.status state machine, Section 10: Known divergences and technical debt (+10 more)
...
Cohesion: 0.11
Nodes (19): class-variance-authority, clsx, @dnd-kit/modifiers, @fontsource-variable/geist, @inertiajs/react, lucide-react, dependencies, class-variance-authority (+11 more)
...
Cohesion: 0.05
Nodes (59): affiliateItems, AppSidebar(), clipItems, NavItem, resolveWorkspace(), SidebarItem, WORKSPACES, NavUser() (+51 more)
...
Cohesion: 0.18
Nodes (15): Handle a usar no crédito: prioriza @handle real; cai para o nome do canal fonte…, resolve_credit_handle(), _fetch_pending_clips(), _has_longo_waiting(), _mark_clip_failed(), _mark_clip_published(), _publish_clips_for(), _publish_one() (+7 more)
...
### Community 82 - "docker-compose.yml (raiz wordpress/)"
Cohesion: 0.14
Nodes (17): clip-processor/Dockerfile, CLIP_PROCESSOR_INTERNAL_TOKEN env var, clip-processor service (build local), CLIPS_DB_PASSWORD como placeholder no SQL, docker-compose.yml (raiz wordpress/), N8N_ENCRYPTION_KEY via ${VAR} nunca hardcoded, n8n service (docker.n8n.io/n8nio/n8n:2.27.0), n8n usa SQLite default (não MySQL) (+9 more)
...
### Community 86 - "telegram-n8n/workflows/06-router.json"
Cohesion: 0.16
Nodes (16): APScheduler dentro do clip-processor (vs n8n cron) para TTL worker, clip-processor/src/processar.py, clip-processor/src/ttl_worker.py, mysql/manual-workflow/approve-backlog.sql, Phase 6 Plan 07 Summary, Phase 6 Context — Controle Manual N8N + Telegram, Phase 6 Research — Telegram bot + n8n orchestration + MySQL schema evolution + Cloudflare Tunnel, Continue Here - Phase 6 Plan 06-07 (Telegram/n8n manual checkpoint) (+8 more)
...
### Community 88 - "_parse_moments"
Cohesion: 0.19
Nodes (9): _clean_reason(), _parse_moments(), Colapsa espaços e corta a justificativa do momento em MAX_REASON_CHARS. Modelo…, Parse JSON text → lista de dicts de momentos com normalização de timestamps., test_selector.py — Testes unitários para selector.py (AI-02, AI-03). Estado…, Justificativa do momento não pode carregar o raciocínio do modelo (visto em…, insert_selected_moments lê moment['reason'] direto — não pode faltar a chave., A limpeza tem que valer para os dois caminhos de IA, que passam por… (+1 more)
...
### Community 92 - "User"
Cohesion: 0.11
Nodes (8): Filament\Models\Contracts\FilamentUser interface, Illuminate\Foundation\Auth\User, Illuminate\Notifications\Notifiable, ResetPainelPassword artisan command, AuthController, User, insertClick(), performanceProps()
...
Cohesion: 0.38
Nodes (7): Brand, BrandMark(), FALLBACK, ICONS, useBrand(), LoginForm(), LoginPage()
...
Cohesion: 0.17
Nodes (10): Usa cliente Anthropic já instanciado (produção ou mock de teste)., Analisa transcrição e retorna momentos selecionados via IA. Args: transcript:…, select_moments(), _select_via_anthropic_client(), Momento com menos de 15 s é descartado: em 3-5 s não há assunto. Esticar 3 s…, Momento de 15 s a 30 s é esticado até MIN_SHORTFORM_SECONDS, não descartado.…, Momento maior que MAX_SHORTFORM_SECONDS (180 s, teto do Shorts) é cortado no…, AI-02: select_moments() retorna lista de dicts com start_time, end_time, score,… (+2 more)
...
### Community 171 - "Illuminate\Http\Request"
Cohesion: 0.08
Nodes (16): Closure, Illuminate\Http\JsonResponse, Illuminate\Http\Request, Illuminate\Http\Response, Inertia\Middleware, OfferApiController, AssistantController, Controller (+8 more)
...
### Community 177 - "clip-processor/src/publisher.py"
Cohesion: 0.24
...
### Community 178 - "Offer"
Cohesion: 0.07
Nodes (8): Illuminate\Database\Eloquent\Factories\HasFactory, Illuminate\Database\Eloquent\Model, Illuminate\Database\Eloquent\Relations\BelongsTo, Illuminate\Database\Eloquent\Relations\HasMany, PublishOffersToTelegram, OfferRedirectController, Offer, OfferClick
...
### Community 192 - "mysql/init/06-multi-canal-migration.sql"
Cohesion: 0.28
...
### Community 196 - "pipeline_runner.py"
Cohesion: 0.31
...
### Community 197 - "Quota, window and round-robin logic"
Cohesion: 0.29
Nodes (7): Quota, window and round-robin logic, Queue does NOT live in Redis (queue = MySQL), QuotaManager per-channel Redis key design, Pitfall: Quota Cega — shared GCP project quota, Content type performance ranking (polêmica > gol > declaração...), Peak publishing hours rule (19h-22h BRT), strategy/CONTENT-CALENDAR.md — publishing schedule strategy

### Community 203 - "TestPollAllChannels"
Cohesion: 0.29
...
### Community 207 - "test_pipeline_runner.py"
Cohesion: 0.40
...
### Community 214 - "04-04 Summary: Poller Integration Summary"
Cohesion: 0.40
...
### Community 220 - "check_download_window_health"
Cohesion: 0.28
...
### Community 221 - "01-04-PLAN: OAuth YouTube e verificação do canal"
Cohesion: 0.40
Nodes (5): youtube/generate_token.py, OAuth app type 'installed' (Desktop App), não 'web', 01-04-PLAN: OAuth YouTube e verificação do canal, 01-04-SUMMARY: OAuth e canal configurados, Canal YouTube "Futebol em Cortes"
...
### Community 225 - "test_uploader.py"
Cohesion: 0.50
...
### Community 229 - "test_uploader_expired.py"
Cohesion: 0.50
Nodes (3): RED test para captura de RefreshError e persistência de oauth_expired_flag…, RefreshError em creds.refresh() deve setar oauth_expired_flag=TRUE em…, test_load_credentials_catches_refresh_error_and_flags_channel()
...
## Knowledge Gaps
- **623 isolated node(s):** `deploy.sh script`, `force-download.sh script`, `$schema`, `style`, `rsc` (+618 more)
...
- **Why does `run_pipeline_once()` connect `run_pipeline_once` to `get_db_connection`, `publish_pending_clips`, `publisher.py`, `pipeline_runner.py`, `notify`, `rss_poller.py`, `n8n/workflows/canaldecortes-pipeline.json`, `_download_pending_videos`, `main.py`, `test_pipeline_runner.py`?**
  _High betweenness centrality (0.087) - this node is a cross-community bridge._
- **Why does `Offer` connect `Offer` to `Inertia\Response`, `Offer`, `Illuminate\Http\Request`, `make_offer`, `User`?**
