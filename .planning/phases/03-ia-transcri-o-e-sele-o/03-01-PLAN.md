...
O ENUM do `generated_clips.status` coloca `pending_cut` como primeiro valor e novo default. Isso é intencional — todo clip criado na Phase 3 começa como `pending_cut`.
  ...
  <verify>
    <automated>docker exec mysql mysql -uroot -pCHANGE_ME_VIA_ENV clips_automation -e "SELECT COLUMN_NAME, COLUMN_TYPE FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME='generated_clips' AND TABLE_SCHEMA='clips_automation' AND COLUMN_NAME IN ('status','reason');" 2>/dev/null | grep -E "pending_cut|reason" || echo "MIGRATION NAO APLICADA — aplicar manualmente se container estiver rodando"</automated>
  </verify>
  <done>
